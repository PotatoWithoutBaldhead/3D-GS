...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, bg, rasterizer = rasterizer)
        ...
        iter_end.record()
        print(f'\nepoch: {iteration}/{opt.iterations} loss:{loss.item():10.8f}')
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), rasterizer)
            ...
            if iteration < opt.densify_until_iter:
                ...
                gaussians.add_densification_stats(rasterizer._p_view, visibility_filter)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc : render, renderArgs, resterizer):
    if tb_writer:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[2000, 3000, 4000, 5000, 6000, 7_000, 8000, 10000, 30_000])
